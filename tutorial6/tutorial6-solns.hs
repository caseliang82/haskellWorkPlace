...
-- Alternative solution that uses fixpoint iteration and
-- works with negative distances and normalises angles.
optimise' :: Command -> Command
optimise' = join' . fix compress . split
  where
    fix :: Eq a => (a -> a) -> a -> a
    fix f x | f x == x  = x
            | otherwise = fix f (f x)
    ...
    compress (Sit    : cs) = cs
    compress (Go 0   : cs) = cs
    compress (Turn 0 : cs) = cs
    -- the next two lines normalise angles to be in the interval [0, 360)
    compress (Turn x : cs) | x >= 360.0 = Turn (x - 360.0) : cs
    compress (Turn x : cs) | x <    0.0 = Turn (x + 360.0) : cs
    compress (Go x   : Go y   : cs) = Go (x+y) : cs
    compress (Turn x : Turn y : cs) = Turn (x+y) : cs
    compress (c      : cs) = c : compress cs
