...
main = do
  ...
  createWindow "Colouring Creativity"
  posi <- newIORef (0, 0, 0)
  gra <- newIORef (0, 0)
  pen <- newIORef 1
...
updateParticle (a,b) p =
				p { particleVely = particleVely p - gravity,
					particlePosx = particlePosx p +a + particleVelx p ,
					particlePosy = particlePosy p + b+ particleVely p,
					...
					particleSize = particleSize p * 1.00}
...
newParticle posi huebaseRef = do
			...
			pvy <- randomRIO (-0.005, 0.005)
			pvz <- randomRIO (0.0001, 0.0001) 
			ptime <- randomRIO (500, 1000)
			return $ Particle { particlePosx = x',
							...
							particleVal = 1,
							particleSize = psize,
							particleVelx = pvx*0.01,
							particleVely = pvy*0.01,
							particleVelz = pvz*0.01 +z',
							particleTTL = ptime}
