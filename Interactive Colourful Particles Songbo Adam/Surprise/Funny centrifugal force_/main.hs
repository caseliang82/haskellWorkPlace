...
main = do
  ...
  createWindow "Funny Centrifugal Fource?"
  plist <- newIORef ([] :: ParticleList)
  currcol <- newIORef (0 :: GLfloat) 
  windowSize $= winSize
...
idle plist currcol = do
			...
			newparticles <- sequence (replicate 20 (newParticle currcol))
			plist $= filter (\p -> particleTTL p >= 0)  (newparticles ++ updateparticles)
			huebase <- get currcol
			currcol $= colmod (huebase)
...
newParticle huebaseRef = do
                        ...
                        pozlen <-  randomRIO (0.05, 0.1)
			huebase <- get huebaseRef
			hue <- randomRIO (huebase,huebase)
