...
-- 1.
{-
a function isBloodOrange :: Fruit -> Bool which returns True for blood oranges 
and False for apples and other oranges. Blood orange varieties are: Tarocco, Moro and Sanguinello. 
-}
isBloodOrange :: Fruit -> Bool
isBloodOrange (Apple _ _) = False
isBloodOrange (Orange v _) = v == "Tarocco" || v == "Moro" || v == "Sanguinello"

-- 2.
{-
a function bloodOrangeSegments :: [Fruit] -> Int which 
returns the total number of blood orange segments in a list of fruit.
-}
bloodOrangeSegments :: [Fruit] -> Int
bloodOrangeSegments fs = sum [s | (Orange v s) <- fs, isBloodOrange (Orange v s)]
--  where seg :: Fruit -> Int
--        seg (Orange v s) = s

-- 3.
{-
a function worms :: [Fruit] -> Int which returns the number of apples that contain worms.
-}
worms :: [Fruit] -> Int
worms fs = length [42 | (Apple _ v) <- fs, v]
